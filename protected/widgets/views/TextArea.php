...
?>
					<?php if (!$nowrapper) {?>
						<div id="div_<?php echo get_class($element)?>_<?php echo $field?>" class="eventDetail"<?php if ($hidden) {?> style="display: none;"<?php }?>>
							...
							<div class="data">
								<?php }?>
								<textarea rows="<?php echo $rows?>" cols="<?php echo $cols?>" name="<?php echo get_class($element)?>[<?php echo $field?>]" id="<?php echo get_class($element)?>_<?php echo $field?>"<?php if (isset($class)) {?> class="<?php echo $class?>"<?php }?><?php if ($disabled){?> disabled="disabled"<?php }?>><?php echo $element->$field?></textarea>
								<?php if (!$nowrapper) {?>
									<?php if ($button) {?>
