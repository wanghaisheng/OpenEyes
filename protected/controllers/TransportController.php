...
class TransportController extends BaseController
{
	...
	public function getTCIEvents($from, $to, $include_bookings, $include_reschedules, $include_cancellations) {
		...
		if ($include_cancellations) {
			if ($sql) $sql .= " UNION ";
			$sql .= "select element_operation.id as eoid, cancelled_booking.id as checkid, patient.id as pid, event.id as evid, patient.first_name, patient.last_name, patient.hos_num, site.short_name as location, element_operation.eye, firm.pas_code as firm, element_operation.decision_date, element_operation.urgent, specialty.ref_spec as specialty, cancelled_booking.date as session_date, cancelled_booking.start_time as session_time, element_operation.status, 'Cancelled' as method, transport_list.id as transport, cancelled_booking.created_date as order_date, 'Unknown' as ward_name from cancelled_booking
	...
	public function actionDownloadcsv() {
		...
		foreach ($data['bookings_all'] as $row) {
			echo '"'.$row['hos_num'].'","'.trim($row['first_name']).'","'.trim($row['last_name']).'","'.$row['session_date'].'","'.$row['session_time'].'","'.$row['location'].'","'.$row['ward_name'].'","'.$row['method'].'","'.$row['firm'].'","'.$row['specialty'].'","'.$row['decision_date'].'","'.($row['urgent'] ? 'Priority' : 'Routine').'"'."\n";
		}
	}
}
