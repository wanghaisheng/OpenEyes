...
$form=$this->beginWidget('CActiveForm', array(
	...
	<div id="search_patient_details" class="form_greyBox clearfix">
		<h3><?php echo Yii::t('strings','Or, search by person details')?></h3>
		<div class="form_column">
			<div class="inputLayout clearfix">
				<?php echo CHtml::label(Yii::t('strings','First name').':<span class="labelRequired">'.Yii::t('strings','First name is required').'</span>', 'first_name');?>
				<?php echo CHtml::textField('Patient[first_name]', '', array('style'=>'width: 150px;', 'class' => 'topPaddingSmall'));?>
			</div>
			<div class="inputLayout clearfix">
				<?php echo CHtml::label(Yii::t('strings','Last name').':<span class="labelRequired">'.Yii::t('strings','Last name is required').'</span>', 'last_name');?>
				<?php echo CHtml::textField('Patient[last_name]', '', array('style'=>'width: 150px;', 'class' => 'topPadding'));?>
			</div>
			<div class="multiInputRight clearfix">
				<?php echo CHtml::label(Yii::t('strings','Date of birth').':<span class="labelHint">dd / mm / yyyy</span>', 'dob');?>
		...
		<div class="form_column">
			<div class="inputLayout clearfix">
				<?php echo CHtml::label(Yii::t('strings','NHS #').':<span class="labelHint">'.Yii::t('strings','for example').': 111-222-3333</span>', 'nhs_number')?>
				<?php echo CHtml::textField('Patient[nhs_num]', '')?>
			</div>
			<div class="customRight clearfix">
				<?php echo CHtml::label(Yii::t('strings','Gender').':<span class="labelHint">'.Yii::t('strings','if known').'</span>', 'gender', array('style'=>'float: left;'));?>
				<?php echo CHtml::radioButtonList('Patient[gender]', '', array('M'=>'Male','F'=>'Female'), array('separator'=>' &nbsp; '))?>
			</div>
		</div>
		<div class="form_button">
			<button type="submit" value="submit" class="btn_find-patient ir" id="findPatient_details"><?php echo Yii::t('strings','Find patient')?></button>
		</div>
	</div>
	<?php $this->endWidget();?>
</form>
