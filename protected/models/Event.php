...
class Event extends BaseActiveRecord
{
	...
	{
		foreach (ElementType::model()->findAll('event_type_id = ?',array($this->eventType->id)) as $element_type) {
			$model = $element_type->class_name;
			$object = $model::model()->find('event_id=?',array($this->id));

			Yii::app()->getController()->renderFile("protected/modules/{$this->eventType->class_name}/views/default/view_{$element_type->class_name}.php",array('element'=>$object));
		}
	}
}
