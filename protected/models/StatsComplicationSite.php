...
/**
 * This is the model class for table "stats_complication_site".
 *
 * The followings are the available columns in table 'stats_complication_site':
 * @property string $id
 ...
 */
class StatsComplicationSite extends BaseActiveRecord {
	/**
	...
	public function tableName() {
		return 'stats_complication_site';
	}
	...
	public function rules() {
		return array(
			array('site_id, event_type_id, element_type_id, complication_id, value_raw, value_total, value_percent', 'safe'),
		);
	}
	...
	public function search() {
		...
		$criteria->compare('site_id',$this->site_id,false);
	...
	static public function add($params) {
		...
		if (!isset($params['site_id'])) throw new Exception('Must specify site_id');
		...
		if (empty($params['complication_id'])) {
			...
			$whereParams = array($params['complication_id']);
		}
		...
		if (!$stat = StatsComplicationSite::model()->find("$complication_where and event_type_id=? and element_type_id=? and site_id=?",$whereParams)) {
			$stat = new StatsComplicationSite;
		}
