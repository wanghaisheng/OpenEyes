...
class GenerateSiteElementTypeDataCommand extends CConsoleCommand
{
	...
	{
		return Yii::t('strings','Generate Site Element Type Data Command');
	}
	public function getHelp()
	{
		return 'site_element_type '.Yii::t('strings','needs to contain all entries which are possible. this command generates those based on other tables');
	}
		...
		foreach ($specialties as $specialty) {
			...
			foreach ($possibleElementTypes as $pet) {
				...
				if (!$existingSiteElementType = SiteElementType::Model()->findAllByAttributes(array('possible_element_type_id' => $pet->id, 'specialty_id' => $specialty->id, 'first_in_episode' => false))) {
					echo "\t".Yii::t('strings','Creating non-first-in-episode site_element_type entry for').": possible_element_type id: " . $pet->id . " specialty: " . $specialty->name . " (event type is: " . $pet->eventType->name . ")\n";
				...
				if ($pet->eventType->first_in_episode_possible == 1) {
					...
					echo "\t".Yii::t('strings','Creating first-in-episode site_element_type entry for').": possible_element_type id: " . $pet->id . " specialty: " . $specialty->name . " (event type is: " . $pet->eventType->name . ")\n";
						$newSiteElementType = new SiteElementType;
						$newSiteElementType->possible_element_type_id = $pet->id;
						$newSiteElementType->specialty_id = $specialty->id;
						$newSiteElementType->view_number = 1;
