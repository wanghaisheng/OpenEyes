...
class m<?php if (isset($migrationid)) { echo $migrationid; } ?>_event_type_<?php echo $this->moduleID; ?> extends CDbMigration
{
	...
	{
<?php if (@$parent_event) {?>
			if (!$parent_event = $this->dbConnection->createCommand()->select("id")->from("event_type")->where("class_name = :class_name",array(":class_name" => '<?php echo $parent_event?>'))->queryRow()) {
				throw new Exception("Parent event type '<?php echo $parent_event?>' not found, please install the parent module first.");
			}
		...
		if (!$this->dbConnection->createCommand()->select('id')->from('event_type')->where('class_name=:class_name', array(':class_name'=>'<?php echo $this->moduleID; ?>'))->queryRow()) {
			...
			$this->insert('event_type', array('class_name' => '<?php echo $this->moduleID;?>', 'name' => '<?php echo $this->moduleSuffix; ?>','event_group_id' => $group['id'],'parent_id' => $parent_id));
		}
